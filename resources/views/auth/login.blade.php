        ...
        <form action="{{ url('login') }}" method="post" class="login-form text-left @if (count($errors) > 0)vivify shake @endif" name="form" id="signup_form">
          ...
          @if($settings->facebook_login == 'on')
            ...
            @if($settings->google_login == 'on')
                <div class="mb-2">
                  <a href="{{url('oauth/google')}}" class="btn btn-block btn-lg google-button no-hover"><img src="{{url('/img/google.svg')}}" width="18" height="18" style="vertical-align: text-bottom;" class="mr-2"> {{trans('misc.sign_in_with')}} Google</a>
                </div>
              @endif
